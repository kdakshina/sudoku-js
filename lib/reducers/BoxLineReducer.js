...
export default class BoxLineReducer extends Reducer {
    ...
    reduceRows(bx, by) {
        for (let deltax = 0; deltax < 2; deltax++) {
            let brow = this.getBoxRow(bx + deltax, by, openCells)
            let restrow = this.getBoxReminingRow(bx + deltax, by, openCells)
    ...
    reduceColumns(bx, by) {
        for (let deltay = 0; deltay < 2; deltay++) {
            let bcol = this.getBoxCol(bx, by + deltay, openCells)
            let restcol = this.getBoxReminingCol(bx, by+deltay, openCells) 
